...
public sealed class Matrix : IFormattable, IEquatable<Matrix>
{
    #region Members
    /// <summary>Storage location</summary>
    readonly double[] _element;
    /// <summary>Get the number of matrix rows</summary>
    public int Rows { get; private set; } = 0;
    /// <summary>Get the number of matrix columns</summary>
    ...
    public Matrix(double[,] m, Matrix fill) : this(m)
    {
        ...
        for (int i = 0; i < fill.Rows; i++)
            for (int j = 0; j < fill.Cols; j++)
                lst.Add(fill[i, j]);
        // If it is 5 columns, then 3/5 is [0 rows, 3 columns]
    ...
    public static Matrix operator +(Matrix a, Matrix b)
    {
        ...
        {
            throw new Exception("The rows and columns of the two matrices are not equal");
        }
    }
    ...
    {
        // Ensure that the original matrix remains unchanged, so clone a copy
        var thisClone = this.Clone();
        if (Cols != Rows || this.Determinant() == 0)
            throw new Exception("The matrix is not a square matrix and cannot be inverted");

        // Initialize an identity matrix of equal size
        var res = thisClone.EMatrix();
        for (int i = 0; i < Rows; i++)
        {
            // First find the number with the largest absolute value in the i-th column, and swap this row with the i-th row
            int rowMax = i;
            ...
            //Exchange the i-th row with the rowMax where the maximum number is found
            if (rowMax != i)
            {
                thisClone.Exchange(i, rowMax);
                res.Exchange(i, rowMax);
            }
            //Perform elementary row transformation on the i-th row, and convert the first non-zero element to 1
    ...
    public double Determinant()
    {
        if (Cols != Rows)
            ...
            throw new Exception("The number is not equal, not a determinant");

        var thisClone = this.Clone();
        //Each time the determinant is swapped, it needs to be multiplied by -1
        double res = 1;
        for (int i = 0; i < Rows; i++)
        {
            //First find the number with the largest absolute value in column i
            int rowMax = i;
            double max = Math.Abs(thisClone[i, i]);
            for (int j = i; j < Rows; j++)
            {
